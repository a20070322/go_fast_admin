...
// Mutate calls f(ctx, m).
func (f AdminMenusFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.AdminMenusMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AdminMenusMutation", m)
	}
	return f(ctx, mv)
}
...
// The AutoUserExampleFunc type is an adapter to allow the use of ordinary
// function as AutoUserExample mutator.
type AutoUserExampleFunc func(context.Context, *ent.AutoUserExampleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AutoUserExampleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.AutoUserExampleMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AutoUserExampleMutation", m)
	}
	return f(ctx, mv)
}
