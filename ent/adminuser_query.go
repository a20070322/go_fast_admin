...
func (auq *AdminUserQuery) QueryRole() *AdminRoleQuery {
	query := &AdminRoleQuery{config: auq.config}
...
// Returns a *NotSingularError when exactly one AdminUser entity is not found.
...
// Returns a *NotSingularError when exactly one AdminUser ID is not found.
...
func (auq *AdminUserQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
func (auq *AdminUserQuery) WithRole(opts ...func(*AdminRoleQuery)) *AdminUserQuery {
	query := &AdminRoleQuery{config: auq.config}
...
//		Scan(ctx, &v)
//
func (auq *AdminUserQuery) Select(field string, fields ...string) *AdminUserSelect {
...
func (auq *AdminUserQuery) sqlAll(ctx context.Context) ([]*AdminUser, error) {
	...
	if query := auq.withRole; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[uuid.UUID]*AdminUser, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(adminuser.RolePrimaryKey[1], fks...))
			},
...
func (auq *AdminUserQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(adminuser.Columns...)...).From(t1)
	if auq.sql != nil {
		selector = auq.sql
		selector.Select(selector.Columns(adminuser.Columns...)...)
	}
...
func (aus *AdminUserSelect) sqlQuery() sql.Querier {
	selector := aus.sql
